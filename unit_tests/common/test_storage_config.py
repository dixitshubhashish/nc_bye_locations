"""Unit tests for storage connection configuration and environment variable loading."""
...
class StorageConfigTests(unittest.TestCase):
    """Test suite validating storage.json parsing and environment variable credential overrides."""

    def test_env_storage_config_works_without_local_json_file(self) -> None:
        """Verify storage config falls back to environment variables when JSON file is absent."""
    ...
    def test_inline_credentials_json_is_written_to_runtime_path(self) -> None:
        """Verify inline JSON service account credentials write to target credential path."""
        with tempfile.TemporaryDirectory() as temp_dir:
    ...
    def test_inline_credentials_accepts_python_dict_style_paste(self) -> None:
        """Verify inline credentials accept Python single-quoted dictionary string representations."""
        with tempfile.TemporaryDirectory() as temp_dir:
            credentials_path = Path(temp_dir) / "service-account.json"
            env = {
                "BIGQUERY_PROJECT_ID": "render-project",
