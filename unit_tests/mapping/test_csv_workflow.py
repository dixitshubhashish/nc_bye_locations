"""Unit tests for CSV data ingestion workflows, mapper validation, and record normalization.

Tests source previews, field discovery, schema validation, quality checks, deduplication, and error listing creation.
"""
...
from whitespace_tool.analytics.analysis import dedupe_locations
...
from whitespace_tool.common.normalization import clean_zip, normalize_location, optional_date, optional_float, optional_int, optional_timestamp
from whitespace_tool.data_validation import validate_normalized_location, validate_source_row
from whitespace_tool.mapping.ingestion import REQUIRED_MAPPER_FIELDS, save_mapper, validate_mapper
from whitespace_tool.persistence.warehouse_bigquery import CONTENT_HASH_FIELDS, TABLE_SCHEMAS, build_table_rows
from whitespace_tool.source_adapters import csv_source, json_source, xml_source
...
class CsvWorkflowTests(unittest.TestCase):
    """Test suite for CSV workflow operations, mapper configuration, and data ingestion."""

    def test_csv_preview_reads_headers_and_rows(self) -> None:
        """Verify csv_source.preview extracts header list and raw rows."""
    ...
    def test_csv_preview_returns_all_rows_for_processing(self) -> None:
        """Verify csv_source.preview returns all rows while capping preview_rows."""
    ...
    def test_preview_field_discovery_scans_all_rows(self) -> None:
        """Verify preview_payload scans all rows to discover columns appearing late in dataset."""
    ...
    def test_csv_preview_removes_blank_headers(self) -> None:
        """Verify csv_source.preview filters out empty or None header strings."""
    ...
    def test_valid_row_normalizes_to_standard_values(self) -> None:
        """Verify normalize_location maps valid raw row attributes to standardized LocationRecord fields."""
    ...
    def test_ratings_field_maps_through_normalization(self) -> None:
        """Verify optional ratings field maps to LocationRecord float value."""
    ...
    def test_ratings_field_is_registered_in_the_standard_field_catalog(self) -> None:
        """Verify ratings field is present in standard field registry definition."""
    ...
    def test_ratings_column_exists_in_listings_schema_and_content_hash(self) -> None:
        """Verify ratings column exists in BigQuery listings schema and content hash calculation."""
    ...
    def test_zip_and_scalar_normalization(self) -> None:
        """Verify helper functions clean zip codes and coerce floats, ints, dates, and timestamps."""
    ...
    def test_invalid_optional_values_are_rejected(self) -> None:
        """Verify validate_source_row flags invalid types in optional fields."""
    ...
    def test_field_validator_registry_is_explicit_and_type_driven(self) -> None:
        """Verify field validators handle currency strings, floats, ints, and dates."""
    ...
    def test_missing_required_mapping_is_reported(self) -> None:
        """Verify validate_mapper flags missing required target field mappings."""
    ...
    def test_unknown_source_mapping_is_reported(self) -> None:
        """Verify validate_mapper flags mapped fields not present in source columns."""
    ...
    def test_missing_required_source_value_is_not_digestible(self) -> None:
        """Verify missing mandatory address value produces empty address in LocationRecord."""
    ...
    def test_normalized_validation_checks_required_and_types(self) -> None:
        """Verify valid LocationRecord passes normalized location validation."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
    ...
    def test_normalized_validation_rejects_invalid_observed_at(self) -> None:
        """Verify validate_normalized_location rejects invalid observed_at string."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
    ...
    def test_dedupe_uses_standard_location_identity_fields(self) -> None:
        """Verify dedupe_locations merges records sharing brand, location_id, address, city, state."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
    ...
    def test_dedupe_drops_fuzzy_address_and_coordinate_matches(self) -> None:
        """Verify dedupe_locations removes fuzzy address and coordinate duplicates."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
    ...
    def test_quality_duplicate_check_uses_standard_location_identity_fields(self) -> None:
        """Verify run_quality_checks identifies duplicate location keys."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
        different_place = dataclasses.replace(
    ...
    def test_bronze_listing_payload_contains_generated_id_and_foreign_keys(self) -> None:
        """Verify build_table_rows attaches listing_id, business_id, and source_type_id to listing dicts."""
        location = normalize_location(VALID_ROW, VALID_MAPPER, "example_csv", 0)
        self.assertIsNotNone(location)
        assert location is not None
    ...
    def test_save_mapper_routes_bad_rows_to_error_listings_without_halting_valid_rows(self) -> None:
        """Verify save_mapper ingests valid rows while isolating bad rows into error_listings."""
        captured_rows = {}
        rows = [
            VALID_ROW,
    ...
    def test_save_mapper_batch_flags_preserve_event_and_skip_duplicate_template(self) -> None:
        """Verify save_mapper retains batch_event_id and respects save_template=False."""
        captured_batches = []
        rows = [
            VALID_ROW,
            {**VALID_ROW, "store_id": "bad-coords", "latitude": "not-a-latitude"},
    ...
    def test_standard_and_optional_fields_keep_expected_modes(self) -> None:
        """Verify schema definitions maintain expected REQUIRED modes on key listing columns."""
    ...
    def test_field_catalogs_are_business_aware_and_plural(self) -> None:
        """Verify field_catalogs table schema includes business_id column."""
    ...
    def test_json_preview_offers_record_layers_through_depth_three(self) -> None:
        """Verify json_source.preview discovers record paths up to nesting depth 3."""
    ...
    def test_json_preview_returns_all_rows_for_processing(self) -> None:
        """Verify json_source.preview returns all rows while capping preview rows."""
    ...
    def test_xml_preview_returns_all_rows_for_processing(self) -> None:
        """Verify xml_source.preview returns all rows while capping preview rows."""
    ...
    def test_geojson_feature_collection_promotes_properties_and_point_coordinates(self) -> None:
        """Verify json_source.preview automatically promotes GeoJSON properties and Point coordinates."""
        content = b"""{
    ...
    def test_geojson_features_record_path_is_mapper_friendly(self) -> None:
        """Verify json_source.preview handles explicit record_path="features" on GeoJSON."""
    ...
    def test_invalid_geographic_and_type_data_flags_error_listings(self) -> None:
        """Verify validate_normalized_location and validate_source_row flag invalid zip codes, coordinates, and types."""
    ...
    def test_currency_formatting_and_domain_boundary_checks(self) -> None:
        """Verify currency strings parse correctly and domain boundary checks flag invalid age/revenue."""
