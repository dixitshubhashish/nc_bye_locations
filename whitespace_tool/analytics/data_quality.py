"""Data quality verification and diagnostic auditing module.

Provides validation checks for missing fields, duplicate location records, invalid geographic codes,
unmatched demographic ZIP coverage, coordinate bounds, and source freshness.
"""
...
from whitespace_tool.analytics.analysis import dedupe_location_key, is_fuzzy_duplicate_location
...
def _issue(severity: str, check: str, message: str, count: int = 1, sample: Any = None) -> dict[str, Any]:
    """Format structured diagnostic issue dictionary for quality audit reporting."""
    return {
        "severity": severity,
        "check": check,
        "message": message,
...
def _parse_observed_at(value: str) -> datetime | None:
    """Parse ISO 8601 timestamp string into UTC aware datetime instance."""
    if not value:
        return None
    try:
...
def _check_source_freshness(config: dict[str, Any], issues: list[dict[str, Any]]) -> None:
    """Audit location source configuration against freshness policy rules."""
...
def run_quality_checks(
    ...
    """Execute complete suite of data quality validation checks across locations and demographics."""
