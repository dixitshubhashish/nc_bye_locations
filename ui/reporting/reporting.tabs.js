/**
 * @file reporting.tabs.js
 * @summary Adds the inner tab strip inside the Reporting screen.
 *
 * @description
 * Injects two sub-tabs into the existing `#reportingView`:
 *   1. "Location Intelligence & Whitespace" - the original report content
 *      (rendered by `reporting.core.js`), and
 *   2. "Data Quality & Improvements" - a quality dashboard this file builds
 *      and populates from the same `/api/reporting` payload.
 *
 * This module is self-contained and self-mounting: it is injected at runtime by
 * `constants.js` (`<script src="/reporting/reporting.tabs.js">`) rather than via
 * a static `<script>` tag, and it polls for `#reportingView` (see the bottom of
 * the file) so it can attach whenever the reporting screen becomes available.
 * It reads two globals published by `reporting.core.js` when present:
 * `window.reportingQueryString` (to share the active filters) and
 * `window.reportingMap` (to re-size the Leaflet map when its tab is shown).
 *
 * The entire file is wrapped in an IIFE; `$`, `num`, `fmt`, and `pct` below are
 * private helpers scoped to it.
 */
(() => {
  /** Shorthand for `document.getElementById`. @param {string} id @returns {HTMLElement|null} */
  const $ = (id) => document.getElementById(id);
  /** Coerce to a finite number, defaulting to 0. @param {*} v @returns {number} */
  const num = (v) => Number.isFinite(Number(v)) ? Number(v) : 0;
  /** Format a number with locale grouping. @param {*} v @returns {string} */
  const fmt = (v) => num(v).toLocaleString();
  /** Format a number as a 1-decimal percentage string. @param {*} v @returns {string} */
  const pct = (v) => `${num(v).toFixed(1)}%`;

  /**
   * Inject this module's scoped CSS (`.reporting-inner-tabs`, `.dq-*`) into
   * `<head>` exactly once. Idempotent - guarded by the style element's id.
   * @returns {void}
   */
  ...
  /**
   * Map a metric to a status CSS class ("good" | "warn" | "bad").
   * @param {*} value - The metric value.
   * @param {number} goodThreshold - Boundary at/above which the value is "good"
   *   (or, when `inverse`, at/below which it is "good").
   * @param {number} warnThreshold - Boundary for the "warn" band.
   * @param {boolean} [inverse=false] - Set true for metrics where lower is
   *   better (e.g. duplicate rate, freshness in days).
  ...
  /**
   * Build the HTML for a single metric card in the quality grid.
   * @param {string|number} value - Headline value shown large.
   * @param {string} label - Caption under the value.
   * @param {string} [note=''] - Optional small print under the label.
   * @returns {string} HTML string for one `.dq-card`.
   */
  ...
  /**
   * Create (once) the "Data Quality & Improvements" panel element with its
   * static scaffolding (intro, status line, metric grid, and section
   * containers that `loadQuality` later fills). Idempotent.
   * @returns {HTMLElement} The quality panel element.
   */
  ...
  /**
   * Fetch `/api/reporting` (honoring the shared filters) and render the data
   * quality dashboard: metric cards, a signals table, derived improvement
   * opportunities, and a reconciliation table. Surfaces any error into the
  ...
  /**
   * Mount the inner tab strip and quality panel into `#reportingView` and wire
   * tab switching. Safe to call repeatedly - it no-ops once mounted.
   * @returns {boolean} `true` once `#reportingView` exists (mounted or already
   *   mounted), `false` if the reporting screen is not in the DOM yet (so the
   *   poller below keeps retrying).
   */
  function init() {
    injectStyles();
    ...
    /**
     * Activate one inner tab: toggle button state, show/hide the location
     * panels vs. the quality panel, lazy-load quality metrics on first view,
     * and re-invalidate the Leaflet map size when returning to the location
     * tab (a map sized while hidden renders blank).
     * @param {"location"|"quality"} name - Tab to activate.
     * @returns {void}
     */
  ...
  // Self-mount: because this script is injected asynchronously, the reporting
  // screen may not exist yet. Poll every 250ms until init() succeeds, giving up
  // after 80 attempts (~20s) to avoid a runaway timer.
  let attempts = 0;
