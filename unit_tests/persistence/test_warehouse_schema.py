"""Unit tests for BigQuery warehouse schema definitions and partition specifications.

Tests partition field matching, sample data metadata, table creation, and bulk loader mechanics.
"""
...
from whitespace_tool.persistence.warehouse_bigquery import TABLE_PARTITION_SPECS, TABLE_SCHEMAS, push_to_bigquery


class WarehouseSchemaTests(unittest.TestCase):
    """Test suite for validating BigQuery table schemas, partition fields, and loading behavior."""

    def test_partition_fields_exist_in_table_schemas(self) -> None:
        """Verify that all fields specified in table partition rules exist in the schema."""
    ...
    def test_business_and_template_source_type_metadata_exists(self) -> None:
        """Verify that source_type_id exists on businesses and workflow_templates tables."""
    ...
    def test_sample_lineage_fields_exist(self) -> None:
        """Verify that sample lineage fields exist across all relevant table schemas."""
    ...
    def test_tables_are_created_without_partitioning(self) -> None:
        """Verify that tables created by push_to_bigquery do not default to time partitioning."""
        created_tables = []
    ...
    def _fake_bigquery_modules_for_skip_check_tests(self, get_table_calls: list, create_table_calls: list):
        """Helper to build fake BigQuery module hierarchy for empty-table check tests."""
        class FakeSchemaField:
            def __init__(self, name: str, field_type: str, mode: str = "NULLABLE", default_value_expression: str | None = None) -> None:
                self.name = name
                self.field_type = field_type
    ...
    def test_skip_empty_table_checks_avoids_get_table_round_trip_for_empty_tables(self) -> None:
        """Verify skip_empty_table_checks=True bypasses get_table calls for empty datasets."""
    ...
    def test_skip_empty_table_checks_defaults_to_false_and_still_checks_empty_tables(self) -> None:
        """Verify skip_empty_table_checks defaults to False and inspects empty tables."""
        get_table_calls: list = []
        create_table_calls: list = []
        modules = self._fake_bigquery_modules_for_skip_check_tests(get_table_calls, create_table_calls)
    ...
    def test_push_uses_dataframe_bulk_loader_with_computed_hash(self) -> None:
        """Verify push_to_bigquery uses DataFrame loader and attaches content_hash for non-JSON tables."""
        loaded_frames = []
    ...
    def test_push_uses_json_loader_for_tables_with_json_columns(self) -> None:
        """Verify push_to_bigquery falls back to JSON loader when tables contain JSON columns."""
        loaded_json_rows = []
...
class PandasIsLazyTests(unittest.TestCase):
    """Test suite for lazy pandas import handling and module dependency isolation."""

    def test_missing_pandas_raises_friendly_error_instead_of_import_crash(self) -> None:
        """Verify that rows_to_dataframe raises a clean RuntimeError when pandas is missing."""
        from whitespace_tool.persistence import warehouse_bigquery
    ...
    def test_workflow_server_module_has_no_top_level_pandas_dependency(self) -> None:
        """Verify that warehouse_bigquery module does not import pandas at top level."""
        import ast

        source_path = Path(__file__).resolve().parent.parent.parent / "whitespace_tool" / "persistence" / "warehouse_bigquery.py"
