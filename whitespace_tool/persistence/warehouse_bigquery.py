"""BigQuery database persistence, table schema management, and batch ingestion.

Defines bronze table schemas, deterministic row content hashing, schema-ordered
DataFrame transformations, JSONL generation, and BigQuery table push/truncate/soft-delete tasks.
"""
...
def _pandas():
    """Lazily import pandas for vectorized row hashing and DataFrame transformations.

    Returns:
        The imported pandas module reference.

    Raises:
        RuntimeError: If pandas or pyarrow dependencies are not installed.
    """
    try:
        import pandas as pd
...
def _canonical_hash_payload(row: dict[str, Any], fields: tuple[str, ...] | list[str]) -> str:
    """Format row field values into a deterministic, sorted JSON payload string.

    Args:
        row: Source dictionary containing field key-value pairs.
        fields: Iterable of field names to include in the payload.

    Returns:
        Sorted JSON string representation of field values.
    """
...
def _is_nullish(value: Any) -> bool:
    """Check if a value is None or pandas NaN/NAT nullish state.
...
def _canonical_hash_payload_from_values(values: list[Any], fields: tuple[str, ...] | list[str]) -> str:
    """Format a list of column values into a sorted JSON payload string.

    Args:
        values: Row value list corresponding positionally to fields.
        fields: Column names list.

    Returns:
        Sorted JSON string representation of non-null field values.
    """
...
def content_hash(row: dict[str, Any]) -> str:
    """Generate a deterministic SHA-256 hex digest over a listing's core content fields.

    Args:
        row: Dictionary containing store location listing attributes.
...
def table_content_hash(table_name: str, row: dict[str, Any]) -> str:
    """Compute deterministic SHA-256 digest across all managed non-hash table columns.

    Args:
        table_name: Managed table schema name in TABLE_SCHEMAS.
        row: Row dictionary containing table fields.
...
def table_columns(table_name: str) -> list[str]:
    """Retrieve ordered list of column names for a managed table schema.

    Args:
        table_name: Target table schema key in TABLE_SCHEMAS.

    Returns:
        List of column field names in schema order.
    """
    return [field["name"] for field in TABLE_SCHEMAS[table_name]]


def table_hash_columns(table_name: str) -> list[str]:
    """Retrieve column list used for computing content_hash for a given table.

    Args:
        table_name: Managed table schema key in TABLE_SCHEMAS.

    Returns:
        List of column names used in hash calculation.
    """
    if table_name == "listings":
...
def table_has_json_fields(table_name: str) -> bool:
    """Determine if a managed table schema includes BigQuery JSON data types.

    Args:
        table_name: Managed table schema key in TABLE_SCHEMAS.

    Returns:
        True if table has any JSON columns, False otherwise.
    """
...
def rows_to_dataframe(table_name: str, rows: list[dict[str, Any]]) -> pd.DataFrame:
    """Build a schema-ordered DataFrame from row dictionaries.

    Args:
        table_name: Managed table schema name.
        rows: List of row dictionaries.

    Returns:
        pandas DataFrame reindexed with exact schema columns.
    """
    pd = _pandas()
...
def dataframe_to_records(frame: pd.DataFrame) -> list[dict[str, Any]]:
    """Convert a DataFrame into BigQuery JSON-compatible row dictionaries.

    Args:
        frame: Input pandas DataFrame.

    Returns:
        List of row dictionaries with NaNs converted to None.
    """
    pd = _pandas()
...
def add_content_hash_column(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Vectorize content hash computation across DataFrame rows.

    Args:
        table_name: Target managed table name.
        frame: pandas DataFrame containing table rows.

    Returns:
        DataFrame populated with content_hash column values.
    """
...
def rows_to_hashed_dataframe(table_name: str, rows: list[dict[str, Any]]) -> pd.DataFrame:
    """Convert raw row dicts into a schema-ordered DataFrame with content_hash populated.

    Args:
        table_name: Managed table schema key.
        rows: List of row dictionary records.

    Returns:
        Fully schema-aligned DataFrame with computed content hashes.
    """
...
def hash_rows_by_table(rows_by_table: dict[str, list[dict[str, Any]]]) -> dict[str, list[dict[str, Any]]]:
    """Compute content hashes for all table datasets in a multi-table payload dictionary.

    Args:
        rows_by_table: Mapping of table names to lists of row dictionaries.

    Returns:
        Updated mapping where each table's rows contain computed content_hash strings.
    """
    return {
...
def _scrub_config(value: Any) -> Any:
    """Scrub sensitive credentials or internal keys from configuration structures."""
...
def _listing_row(row: LocationRecord) -> dict[str, Any]:
    """Convert a LocationRecord dataclass instance into a BigQuery listings table row dictionary.

    Args:
        row: LocationRecord instance to convert.

    Returns:
        Row dictionary aligned with listings table schema.
    """
    listing = {
...
def build_table_rows(
    ...
    """Transform locations and demographics into structured, hashed rows across managed tables.

    Args:
        locations: Normalized store location records.
        demographics: ZIP code demographics dictionary.
        config: Optional run configuration parameters.
        whitespace_rows: Optional pre-calculated gap analysis rows.
        summary: Optional summary metadata.

    Returns:
        Dictionary mapping table names to lists of schema-aligned row dictionaries.
    """
...
def write_bigquery_jsonl(output_dir: str | Path, rows_by_table: dict[str, list[dict[str, Any]]]) -> None:
    """Export table row datasets to newline-delimited JSONL files.

    Args:
        output_dir: Destination folder path for output JSONL files.
        rows_by_table: Dictionary of table names to row records lists.
    """
    out = Path(output_dir)
...
def write_bigquery_schema(output_dir: str | Path) -> None:
    """Export BigQuery table schemas to JSON specification files.

    Args:
        output_dir: Destination directory path for schema JSON files.
    """
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
...
def push_to_bigquery(
    ...
    """Batch load table datasets into Google Cloud BigQuery bronze dataset.

    Args:
        project_id: GCP project identifier.
        dataset_id: BigQuery bronze dataset name.
        rows_by_table: Dictionary mapping table names to lists of row dictionaries.
        credentials_json: Optional path to service account credentials file.
        write_disposition: BigQuery job write disposition (e.g., WRITE_TRUNCATE, WRITE_APPEND).
        client: Optional pre-initialized BigQuery client instance.
        skip_empty_table_checks: If True, skip table checks when row list is empty.
    """
    try:
...
def _clear_dataset_tables_with_client(client: Any, dataset_ref: str) -> dict[str, list[str]]:
    """Execute table clearing or soft-deletion using an active BigQuery client.

    Args:
        client: BigQuery client instance.
        dataset_ref: Target BigQuery dataset reference string.

    Returns:
        Dictionary summarizing soft-deleted and truncated table names.
    """
...
def clear_dataset_tables(
    ...
    """Soft-delete listings/businesses and truncate auxiliary dataset tables.

    Args:
        project_id: GCP project ID.
        dataset_id: BigQuery dataset ID to clean up.
        credentials_json: Optional credentials file path.

    Returns:
        Dictionary summarizing soft-deleted and truncated table lists.
    """
    try:
        from google.cloud import bigquery
        from google.oauth2 import service_account
