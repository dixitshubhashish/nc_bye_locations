"""Command Line Interface (CLI) runner module for whitespace tool workflows.

Provides CLI subcommands for analysis execution, public ZIP fetching, data quality checks,
BigQuery staging/pushing, serving the workflow web UI, and Domino's store fetching.
"""
...
from whitespace_tool.analytics.analysis import analyze_whitespace
from whitespace_tool.analytics.data_quality import run_quality_checks
from whitespace_tool.common.config import load_config
...
def run_analysis(config_path: str, output_dir: str) -> None:
    """Execute complete whitespace analysis pipeline and write outputs to disk.

    Args:
        config_path: Path to run configuration JSON.
        output_dir: Target directory for CSV/JSON outputs.
    """
...
def fetch_public_zips(config_path: str, output_path: str, limit: int | None) -> None:
    """Fetch US ZIP geography and demographics from BigQuery public data.

    Args:
        config_path: Path to storage connection config.
...
def quality_check(config_path: str, output_dir: str) -> None:
    """Run data quality assertions and save quality report.

    Args:
        config_path: Path to configuration file.
        output_dir: Target directory for quality report files.
    """
    config = load_config(config_path)
    locations = load_location_sources(config)
    demographics = load_demographics(config)
...
def push_bigquery(
    ...
    """Stage or push unified location tables to BigQuery warehouse.

    Args:
        config_path: Configuration path.
        project_id: BigQuery project ID override.
        dataset_id: BigQuery dataset ID override.
        stage_dir: Staging output directory.
        dry_run: If True, skips network push.
        credentials_json: Optional credentials JSON path override.
    """
    config = load_config(config_path)
...
def serve_workflow_ui(host: str, port: int) -> None:
    """Start web server for the no-code workflow UI.

    Args:
        host: Host interface binding.
        port: Listening port.
    """
...
def fetch_dominos(zip_file: str, output: str, cache_dir: str, order_type: str) -> None:
    """Fetch Domino's store locations for a set of ZIP codes.

    Args:
        zip_file: File path with ZIP codes.
        output: Target JSON output path.
        cache_dir: Response cache directory.
        order_type: Order type parameter ('Carryout' or 'Delivery').
    """
...
def main() -> None:
    """CLI entry point parsing command line options and invoking subcommands."""
    parser = build_parser()
...
run_fetch_dominos = fetch_dominos
run_fetch_public_zips = fetch_public_zips
run_push_bigquery = push_bigquery
run_quality_check = quality_check
run_workflow_ui = serve_workflow_ui
