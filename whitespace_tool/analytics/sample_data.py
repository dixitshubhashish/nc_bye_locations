"""Sample dataset generators and brand configuration registry.

Provides sample brand metadata, geographic reference points, row generators, mapper schema builders,
and deterministic synthetic error injection for testing and demonstration workflows.
"""
...
class SampleBrandConfig:
    """Configuration metadata for synthetic sample brand generation."""
...
def stable_business_id(key: str) -> str:
    """Generate deterministic business_id string for a given sample brand key."""
    return f"sample_business_{key}"


def stable_template_id(key: str) -> str:
    """Generate deterministic template_id string for a given sample brand key."""
    return f"sample_template_{key}"


def source_label(source_type: str) -> str:
    """Return user-facing uppercase source label for a given source type identifier."""
...
def source_configuration(config: SampleBrandConfig) -> dict[str, Any]:
    """Build source configuration dictionary for a sample brand configuration."""
...
def mapper_for(config: SampleBrandConfig, business_id: str, source_type_id: str) -> dict[str, Any]:
    """Construct field mapping dictionary for a given brand and source type."""
    fields_by_type = {
        "csv": {
...
def _geo_for(config: SampleBrandConfig, index: int) -> dict[str, Any]:
    """Select geographic reference point matching brand geography constraints."""
...
def _base_row(config: SampleBrandConfig, index: int, rng: random.Random) -> dict[str, Any]:
    """Construct base location attribute dictionary with random jittered coordinates."""
...
def _shape_row(config: SampleBrandConfig, base: dict[str, Any]) -> dict[str, Any]:
    """Format base location dictionary into source-specific JSON structure."""
...
def _inject_error(config: SampleBrandConfig, row: dict[str, Any], index: int) -> None:
    """Inject intentional data quality errors into a row based on error rate."""
    if config.source_type == "csv":
        if index % 2:
            row["postal_code"] = ""
        else:
...
def generate_source_rows(config: SampleBrandConfig, source_type_id: str, batch_id: str = SAMPLE_BATCH_ID) -> list[dict[str, Any]]:
    """Generate list of shaped sample rows complete with metadata and intentional errors."""
