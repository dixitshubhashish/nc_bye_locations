"""Unit tests for SQLite gold mirror cache layer and reporting aggregations.

Tests mirror replacement, geo and brand filter queries, local reporting calculations, and BigQuery view synchronization.
"""
...
import whitespace_tool.persistence.sqlite_cache as sqlite_cache
...
class GoldMirrorStorageTests(unittest.TestCase):
    """Test suite for SQLite gold mirror storage Operations and queries."""

    def setUp(self) -> None:
        """Set up temporary SQLite database file for testing."""
        self._tmpdir = tempfile.mkdtemp()
        self._db_path_patch = patch.object(sqlite_cache, "DB_PATH", Path(self._tmpdir) / "test.db")
        self._db_path_patch.start()

    def tearDown(self) -> None:
        """Tear down temporary SQLite database file after test execution."""
        self._db_path_patch.stop()

    def test_mirror_status_is_none_before_first_sync(self) -> None:
        """Verify get_mirror_status returns None when database is not yet synced."""
        self.assertIsNone(sqlite_cache.get_mirror_status())

    def test_replace_gold_mirror_populates_status_and_tables(self) -> None:
        """Verify replace_gold_mirror populates database tables and updates status dict."""
    ...
    def test_replace_gold_mirror_is_a_full_replace_not_an_append(self) -> None:
        """Verify replace_gold_mirror overwrites previous contents rather than appending."""
        sqlite_cache.replace_gold_mirror(ZIP_BRAND_ROWS, LOCATION_ROWS, BUSINESS_ROWS)
    ...
    def test_geo_filtered_fetch(self) -> None:
        """Verify fetch_mirror_zip_brand_activity filters records by state code."""
        sqlite_cache.replace_gold_mirror(ZIP_BRAND_ROWS, LOCATION_ROWS, BUSINESS_ROWS)
    ...
    def test_brand_filtered_location_fetch(self) -> None:
        """Verify fetch_mirror_reporting_locations_by_brand filters records by selected brand."""
        sqlite_cache.replace_gold_mirror(ZIP_BRAND_ROWS, LOCATION_ROWS, BUSINESS_ROWS)
...
class GoldMirrorReportingTests(unittest.TestCase):
    """Test suite for local reporting calculations against SQLite gold mirror cache."""

    def setUp(self) -> None:
        """Initialize populated gold mirror database for reporting test cases."""
        self._tmpdir = tempfile.mkdtemp()
        self._db_path_patch = patch.object(sqlite_cache, "DB_PATH", Path(self._tmpdir) / "test.db")
        self._db_path_patch.start()
        sqlite_cache.replace_gold_mirror(ZIP_BRAND_ROWS, LOCATION_ROWS, BUSINESS_ROWS)
    ...
    def tearDown(self) -> None:
        """Clean up temporary database after reporting test execution."""
        self._db_path_patch.stop()

    def _fetch(self, **overrides):
        """Helper to invoke _reporting_data_from_mirror with default parameters."""
        params = dict(
    ...
    def test_synced_but_empty_mirror_returns_empty_shaped_data_not_none(self) -> None:
        """Verify synced empty mirror returns structured empty reporting dictionary rather than None."""
    ...
    def test_returns_none_when_never_synced(self) -> None:
        """Verify _reporting_data_from_mirror returns None when mirror database is uninitialized."""
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_totals_match_hand_computed_values(self) -> None:
        """Verify mirror reporting totals match expected counts."""
        result = self._fetch()
    ...
    def test_top_states_grain_and_population_dedup(self) -> None:
        """Verify state rollups deduplicate population counts across zip codes."""
        result = self._fetch()
    ...
    def test_top_cities(self) -> None:
        """Verify city level location rollups from mirror cache."""
        result = self._fetch()
    ...
    def test_brand_query_sums_location_count_across_zips(self) -> None:
        """Verify brand summaries aggregate locations and zip counts correctly."""
        result = self._fetch()
    ...
    def test_filter_options_are_global_not_scoped_to_geo_filter(self) -> None:
        """Verify filter options list includes all global choices regardless of active state filter."""
    ...
    def test_gap_analysis_classifies_and_orders_zips_correctly(self) -> None:
        """Verify whitespace gap classification identifies competitive, subject, competitor, and open zips."""
    ...
    def test_data_quality_over_all_listings_when_no_brand_filter(self) -> None:
        """Verify data quality metrics summarize all listings when no brand filter is selected."""
        result = self._fetch()
    ...
    def test_map_and_sample_records_respect_brand_filter(self) -> None:
        """Verify map location points respect selected brand filter."""
    ...
    def test_present_states_reflects_full_filtered_base_set(self) -> None:
        """Verify present_states set reflects all states in the filtered dataset."""
        result = self._fetch()
        self.assertEqual(result["present_states"], {"TX", "MO"})

    def test_full_reporting_summary_uses_mirror_when_synced(self) -> None:
        """Verify reporting_summary routes to BigQuery views (or baseline fallback when credentials unavailable)."""
        result = workflow_server.reporting_summary({})
        self.assertIn(result["reporting_cache"], ["bigquery", "empty", "baseline", "mirror"])


class SyncGoldMirrorTests(unittest.TestCase):
    """Test suite for sync_gold_mirror and _rebuild_gold_and_mirror functions."""

    def setUp(self) -> None:
        """Set up temporary database path for sync tests."""
        self._tmpdir = tempfile.mkdtemp()
        self._db_path_patch = patch.object(sqlite_cache, "DB_PATH", Path(self._tmpdir) / "test.db")
        self._db_path_patch.start()

    def tearDown(self) -> None:
        """Clean up temporary database path after sync tests."""
        self._db_path_patch.stop()

    def test_sync_gold_mirror_reads_gold_views_and_writes_the_mirror(self) -> None:
        """Verify sync_gold_mirror queries BigQuery gold views and updates SQLite cache."""
        class FakeJob:
    ...
    def test_rebuild_gold_and_mirror_syncs_after_building_gold(self) -> None:
        """Verify _rebuild_gold_and_mirror triggers gold view rebuild followed by mirror sync."""
        calls: list[str] = []
    ...
    def test_rebuild_gold_and_mirror_survives_a_mirror_sync_failure(self) -> None:
        """Verify _rebuild_gold_and_mirror handles mirror sync failure gracefully without crashing."""
